...
BEGIN
    ...
    THEN
        ...
        EXECUTE FORMAT(
                'update %I.%I set valid_to = %3$s, is_active = false where uid = ($1).uid and version = ($1).version',
                audit_schema_name, audit_table_name, quote_literal(ts))
            USING OLD;
...
BEGIN
    EXECUTE FORMAT(
            'CREATE TABLE if not exists audit.%1$I  (valid_from timestamp, valid_to timestamp, audit_active boolean);',
            tablename);
